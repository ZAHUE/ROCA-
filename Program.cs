...
namespace ROCAPointBot
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            app.MapGet("/", () => "✅ ROCA Point Bot 正在運行中！(Roblox DataStore 主資料同步模式)");

            app.MapGet("/api/points/pending", async (HttpRequest request, IConfiguration config) =>
            {
                if (!IsValidSyncSecret(request, config)) return Results.Unauthorized();

                using var db = new BotDbContext(config);
                var takeRaw = request.Query["take"].FirstOrDefault();
                var defaultTake = int.TryParse(config["RobloxSync:PendingTakeDefault"], out var configuredTake) ? configuredTake : 20;
                var take = int.TryParse(takeRaw, out var parsedTake) ? parsedTake : defaultTake;
                take = Math.Clamp(take, 1, 100);

                var now = GetTaipeiTime();
                var staleAt = now.AddMinutes(-2);
                var items = await db.PointSyncRequests
                    .Where(x => x.Status == "Pending" || (x.Status == "Processing" && x.PickedAt != null && x.PickedAt <= staleAt))
            ...
            app.MapGet("/pending", async (HttpRequest request, IConfiguration config) =>
            {
                if (!IsValidSyncSecret(request, config)) return Results.Unauthorized();
            ...
            app.MapPost("/api/points/ack", async (HttpRequest request, IConfiguration config, PointSyncAckPayload payload) =>
            {
                if (!IsValidSyncSecret(request, config)) return Results.Unauthorized();
                if (string.IsNullOrWhiteSpace(payload.RequestId)) return Results.BadRequest(new { message = "requestId required" });

                using var db = new BotDbContext(config);
                var syncRequest = await db.PointSyncRequests.FirstOrDefaultAsync(x => x.RequestId == payload.RequestId);
                if (syncRequest == null) return Results.NotFound(new { message = "request not found" });

                var now = GetTaipeiTime();
                syncRequest.AckPayloadJson = JsonSerializer.Serialize(payload);
                syncRequest.CompletedAt = now;

                db.PointSyncAudits.Add(new PointSyncAudit
                {
                    RequestId = payload.RequestId,
                    Success = payload.Success,
                    Message = payload.Message ?? string.Empty,
                    PayloadJson = syncRequest.AckPayloadJson,
                    CreatedAt = now
                });

                if (payload.Success)
                {
                    syncRequest.Status = "Applied";
                    syncRequest.ErrorMessage = null;
                    await PointSyncMirrorHelper.ApplyMirrorUpdateFromAckAsync(db, config, syncRequest, payload);
                    await db.SaveChangesAsync();
                    await GoogleSheetSyncHelper.RunAsync(config, syncRequest.GuildId);
                    return Results.Ok(new { message = "ack applied" });
                }

                syncRequest.Status = "Failed";
                syncRequest.ErrorMessage = payload.Message;
                await db.SaveChangesAsync();
                return Results.Ok(new { message = "ack recorded as failed" });
            });

            app.MapPost("/ack", async (HttpRequest request, IConfiguration config, PointSyncAckPayload payload) =>
            {
                if (!IsValidSyncSecret(request, config)) return Results.Unauthorized();
                if (string.IsNullOrWhiteSpace(payload.RequestId)) return Results.BadRequest(new { message = "requestId required" });
        ...
        public static bool IsValidSyncSecret(HttpRequest req, IConfiguration config)
        {
            var expected = config["RobloxSync:Secret"];
            if (string.IsNullOrWhiteSpace(expected)) return false;
            if (!req.Headers.TryGetValue("X-ROCA-SECRET", out var provided)) return false;
    ...
    public class DiscordBotService : BackgroundService
    {
        ...
        private static string ExtractRobloxName(SocketGuildUser user)
        {
        ...
        private static string NormalizePointType(string? rawPointType)
        {
            return string.Equals(rawPointType, "OEP", StringComparison.OrdinalIgnoreCase) ? "OEP" : "EP";
        }

        private static string GetDepartmentKey(BotConfig? config)
        {
            return string.IsNullOrWhiteSpace(config?.RobloxGroupId) ? "UNBOUND" : config!.RobloxGroupId.Trim();
        }

        private static string GetDepartmentDisplayName(BotConfig? config)
        {
            return GetDepartmentDisplayNameByKey(config?.RobloxGroupId);
        }

        private static string GetDepartmentDisplayNameByKey(string? departmentKey)
        {
            if (string.IsNullOrWhiteSpace(departmentKey)) return "未綁定單位";
        ...
        private static int GetDepartmentTotal(DepartmentPoint point)
        {
            return (point?.EP ?? 0) + (point?.OEP ?? 0);
        }

        private static int GetPointTypeValue(DepartmentPoint point, string pointType)
        {
            return NormalizePointType(pointType) == "OEP" ? point?.OEP ?? 0 : point?.EP ?? 0;
        }

        private static void ApplyDepartmentPointDelta(DepartmentPoint point, string pointType, int delta)
        {
            if (NormalizePointType(pointType) == "OEP") point.OEP = Math.Max(0, point.OEP + delta);
            else point.EP = Math.Max(0, point.EP + delta);
        }

        private async Task<(bool exists, string? name, string? errorMessage)> GetRobloxGroupInfoAsync(string groupId)
        {
            if (string.IsNullOrWhiteSpace(groupId) || !long.TryParse(groupId, out _))
            {
                return (false, null, "❌ Roblox Group ID 格式錯誤，請輸入純數字 ID。");
            }

            try
            {
                var res = await _http.GetAsync($"https://groups.roblox.com/v1/groups/{groupId}");
                if (res.StatusCode == System.Net.HttpStatusCode.NotFound)
                {
                    return (false, null, $"❌ 找不到 Roblox 群組 `{groupId}`，請確認 ID 是否正確。");
                }
                if (!res.IsSuccessStatusCode)
                {
                    return (false, null, $"❌ Roblox 群組驗證失敗 (HTTP {(int)res.StatusCode})，請稍後再試。");
                }

                using var doc = JsonDocument.Parse(await res.Content.ReadAsStringAsync());
                string? name = doc.RootElement.TryGetProperty("name", out var nameProp) ? nameProp.GetString() : null;
                return (true, name, null);
            }
            catch (Exception ex)
            {
                return (false, null, $"❌ 無法驗證 Roblox 群組：{ex.Message}");
            }
        }

        private async Task<UserPoint> EnsureUserPointAsync(BotDbContext db, ulong guildId, string robloxUsername)
        {
            var userPoint = await db.UserPoints.FirstOrDefaultAsync(u => u.GuildId == guildId && u.RobloxUsername.ToLower() == robloxUsername.ToLower());
            if (userPoint == null)
            {
                userPoint = new UserPoint { GuildId = guildId, RobloxUsername = robloxUsername, Points = 0 };
                db.UserPoints.Add(userPoint);
            }
            return userPoint;
        }

        private async Task<DepartmentPoint> GetOrCreateDepartmentPointAsync(BotDbContext db, ulong guildId, string robloxUsername, string departmentKey)
        {
            var departmentPoint = await db.DepartmentPoints.FirstOrDefaultAsync(p => p.GuildId == guildId && p.DepartmentKey == departmentKey && p.RobloxUsername.ToLower() == robloxUsername.ToLower());
            if (departmentPoint == null)
            {
                departmentPoint = new DepartmentPoint { GuildId = guildId, RobloxUsername = robloxUsername, DepartmentKey = departmentKey, EP = 0, OEP = 0 };
                db.DepartmentPoints.Add(departmentPoint);
            }
            return departmentPoint;
        }

        private void SyncLegacyUserPointTotal(UserPoint userPoint, DepartmentPoint departmentPoint)
        {
            userPoint.Points = GetDepartmentTotal(departmentPoint);
        }

        private void MigrateLegacyPointsIfNeeded(UserPoint userPoint, DepartmentPoint departmentPoint)
        {
            if (GetDepartmentTotal(departmentPoint) == 0 && userPoint.Points > 0)
            {
                departmentPoint.EP = userPoint.Points;
            }
            SyncLegacyUserPointTotal(userPoint, departmentPoint);
        }

        private async Task<DepartmentPoint> GetDepartmentPointSnapshotAsync(BotDbContext db, BotConfig? config, ulong guildId, string robloxUsername)
        {
            var userPoint = await EnsureUserPointAsync(db, guildId, robloxUsername);
            var departmentPoint = await GetOrCreateDepartmentPointAsync(db, guildId, robloxUsername, GetDepartmentKey(config));
            MigrateLegacyPointsIfNeeded(userPoint, departmentPoint);
            return departmentPoint;
        }


        private static string ResolveDepartmentName(string departmentKey)
        {
            return GetDepartmentDisplayNameByKey(departmentKey);
        }

        private async Task<(bool success, long userId, string? error)> TryResolveRobloxUserIdAsync(string robloxUsername)
        {
            try
            {
                var userReq = new { usernames = new[] { robloxUsername }, excludeBannedUsers = true };
                ...
                {
                    return (false, 0, $"Roblox users API 驗證失敗 (HTTP {(int)userRes.StatusCode})。");
                }

                using var doc = JsonDocument.Parse(await userRes.Content.ReadAsStringAsync());
                var data = doc.RootElement.GetProperty("data");
                if (data.GetArrayLength() == 0)
                {
                    return (false, 0, "找不到對應的 Roblox 使用者名稱。");
                }

                return (true, data[0].GetProperty("id").GetInt64(), null);
            }
            catch (Exception ex)
            {
        ...
        private async Task<PointSyncRequest> CreatePointSyncRequestAsync(
            BotDbContext db,
            ulong guildId,
            ...
            string source,
            int? relatedPointLogId = null)
        {
            var request = new PointSyncRequest
            {
                GuildId = guildId,
                ...
                PointType = NormalizePointType(pointType),
                Amount = amount,
                Reason = reason,
                EditorName = editorName,
                EditorDiscordUserId = editorDiscordUserId,
                Source = source,
                Status = "Pending",
                CreatedAt = Program.GetTaipeiTime(),
                RelatedPointLogId = relatedPointLogId
            };

            db.PointSyncRequests.Add(request);
            await db.SaveChangesAsync();
            return request;
        }

        private static string FormatPointBreakdown(DepartmentPoint departmentPoint)
        {
            return $"EP: {departmentPoint.EP} / OEP: {departmentPoint.OEP} / 合計: {GetDepartmentTotal(departmentPoint)}";
        }
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD PointType NVARCHAR(10) NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD PointType TEXT NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD DepartmentKey NVARCHAR(100) NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD DepartmentKey TEXT NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD PendingRevertRequestId NVARCHAR(64) NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD PendingRevertRequestId TEXT NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD RevertRequestedAt datetime2 NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD RevertRequestedAt DATETIME NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD RevertedAt datetime2 NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD RevertedAt DATETIME NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD SyncRequestId NVARCHAR(64) NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD SyncRequestId TEXT NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD MirrorSource NVARCHAR(64) NULL"); } catch { }
                    try { await db.Database.ExecuteSqlRawAsync("ALTER TABLE PointLogs ADD MirrorSource TEXT NULL"); } catch { }
                    ...
                    {
                        await db.Database.ExecuteSqlRawAsync(@"
                        IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'DepartmentPoints')
                        BEGIN
                            CREATE TABLE DepartmentPoints (
                                Id int IDENTITY(1,1) PRIMARY KEY,
                                GuildId decimal(20,0) NOT NULL,
                                RobloxUsername nvarchar(255) NOT NULL,
                                DepartmentKey nvarchar(100) NOT NULL,
                                EP int NOT NULL DEFAULT(0),
                                OEP int NOT NULL DEFAULT(0)
                            )
                        END");
                    }
                    catch { }
                    try { await db.Database.ExecuteSqlRawAsync("CREATE TABLE IF NOT EXISTS DepartmentPoints (Id INTEGER PRIMARY KEY AUTOINCREMENT, GuildId INTEGER NOT NULL, RobloxUsername TEXT NOT NULL, DepartmentKey TEXT NOT NULL, EP INTEGER NOT NULL DEFAULT 0, OEP INTEGER NOT NULL DEFAULT 0)"); } catch { }
                    try
                    {
                        await db.Database.ExecuteSqlRawAsync(@"
                        IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'PointSyncRequests')
                        BEGIN
                            CREATE TABLE PointSyncRequests (
                                ...
                                RobloxUsername nvarchar(255) NOT NULL,
                                PointType nvarchar(16) NOT NULL,
                                Amount decimal(18,2) NOT NULL,
                                Reason nvarchar(max) NOT NULL,
                                EditorName nvarchar(255) NOT NULL,
                                EditorDiscordUserId decimal(20,0) NULL,
                                ...
                                AckPayloadJson nvarchar(max) NULL,
                                RetryCount int NOT NULL DEFAULT(0),
                                RelatedPointLogId int NULL
                            )
                        END");
                    }
                    catch { }
                    try { await db.Database.ExecuteSqlRawAsync("CREATE TABLE IF NOT EXISTS PointSyncRequests (RequestId TEXT PRIMARY KEY, GuildId INTEGER NOT NULL, DepartmentKey TEXT NOT NULL, DepartmentName TEXT NOT NULL, RobloxUserId INTEGER NOT NULL, RobloxUsername TEXT NOT NULL, PointType TEXT NOT NULL, Amount REAL NOT NULL, Reason TEXT NOT NULL, EditorName TEXT NOT NULL, EditorDiscordUserId INTEGER NULL, Source TEXT NOT NULL, Status TEXT NOT NULL, CreatedAt DATETIME NOT NULL, PickedAt DATETIME NULL, CompletedAt DATETIME NULL, ErrorMessage TEXT NULL, AckPayloadJson TEXT NULL, RetryCount INTEGER NOT NULL DEFAULT 0, RelatedPointLogId INTEGER NULL)"); } catch { }
                    try
                    {
                        await db.Database.ExecuteSqlRawAsync(@"
                        IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'PointSyncAudits')
                        BEGIN
                            CREATE TABLE PointSyncAudits (
                                Id int IDENTITY(1,1) PRIMARY KEY,
                    ...
                    try { await db.Database.ExecuteSqlRawAsync("CREATE TABLE IF NOT EXISTS PointSyncAudits (Id INTEGER PRIMARY KEY AUTOINCREMENT, RequestId TEXT NOT NULL, Success INTEGER NOT NULL, Message TEXT NOT NULL, PayloadJson TEXT NOT NULL, CreatedAt DATETIME NOT NULL)"); } catch { }
                    // 👇 ================= 新增這段程式碼 ================= 👇
        ...
        private async Task RegisterCommandsAsync()
        {
            var commands = new List<ApplicationCommandProperties>
            {
                ...
                .AddOption(new SlashCommandOptionBuilder()
                    ...
                    .WithDescription("輸入要綁定的 Roblox Group ID（可填 ROCA 外的自訂群組）")
                ...
                new SlashCommandBuilder().WithName("sync-status").WithDescription("📡 查詢單筆點數同步請求狀態").AddOption("request_id", ApplicationCommandOptionType.String, "要查詢的同步請求編號", isRequired: true).Build(),
                ...
                new SlashCommandBuilder().WithName("addpoint").WithDescription("➕ 發放部門點數")
              ...
              new SlashCommandBuilder().WithName("add-menu-item")
                ...
                new SlashCommandBuilder().WithName("end-event")
                    ...
                    .AddOption(new SlashCommandOptionBuilder()
                        .WithName("point_type")
                        .WithDescription("選擇此次活動要發放的點數類型")
                        .WithType(ApplicationCommandOptionType.String)
                        .WithRequired(false)
                        .AddChoice("EP", "EP")
                        .AddChoice("OEP", "OEP"))
                ...
                .Build(),
                new SlashCommandBuilder().WithName("apply-points")
                .WithDescription(" 提交點數發放申請 (需附證明連結)")
                .AddOption(new SlashCommandOptionBuilder()
                    .WithName("point_type")
                    .WithDescription("選擇申請的點數類型")
                    .WithType(ApplicationCommandOptionType.String)
                    .WithRequired(false)
                    .AddChoice("EP", "EP")
                    .AddChoice("OEP", "OEP"))
                .Build(),
                new SlashCommandBuilder().WithName("apply-redeem")
                .WithDescription(" 提交點數兌換申請 (兌換 MENU 內容)")
                .AddOption(new SlashCommandOptionBuilder()
                    .WithName("point_type")
                    .WithDescription("選擇此次兌換要扣除的點數類型")
                    .WithType(ApplicationCommandOptionType.String)
                    .WithRequired(false)
                    .AddChoice("EP", "EP")
                    .AddChoice("OEP", "OEP"))
            ...
            try
            {
                ...
                if (command.Data.Name != "apply-points" && command.Data.Name != "apply-redeem")
                {
                    ...
                    bool isEphemeral = command.Data.Name == "setup-roca" || command.Data.Name == "unbind-roca" ||
                                       ...
                                       command.Data.Name == "end-event" || command.Data.Name == "sync-status";
                ...
                switch (command.Data.Name)
                {
                    case "setup-roca":
                        {
                            ...
                            string rId = ((string)command.Data.Options.First(x => x.Name == "roblox_group_id").Value).Trim();
                            var groupInfo = await GetRobloxGroupInfoAsync(rId);
                            if (!groupInfo.exists)
                            {
                                await command.FollowupAsync(groupInfo.errorMessage ?? "❌ 無法驗證 Roblox 群組。", ephemeral: true);
                                return;
                            }
                            ...
                            string verifiedGroupName = string.IsNullOrWhiteSpace(groupInfo.name) ? "未知群組" : groupInfo.name;
                            await command.FollowupAsync($"✅ 已綁定 Roblox 群組 `{rId}`（{verifiedGroupName}）。\n🔑 **本伺服器專屬資料庫編號：`{botConfig.ServerCode}`** (請妥善保存，交給國防部設定 Admin 頻道使用)。\n正在背景抓取群組成員，請稍候...");
                    ...
                    case "end-event":
                        {
                            ...
                            string pointType = NormalizePointType(command.Data.Options.FirstOrDefault(x => x.Name == "point_type")?.Value?.ToString());
                            ...
                            _pendingDistributions[sessId] = new PendingEventDistribution
                            {
                                ...
                                Points = points,
                                PointType = pointType,
                                DepartmentKey = GetDepartmentKey(botConfig),
                                Reason = finalReason,
                                AdminId = exec.Id,
                    ...
                    case "viewall":
                        {
                            ...
                            string departmentKey = GetDepartmentKey(botConfig);
                            var all = (await db.DepartmentPoints.Where(u => u.GuildId == gid && u.DepartmentKey == departmentKey).ToListAsync())
                                .OrderByDescending(u => u.EP + u.OEP)
                                .ThenBy(u => u.RobloxUsername)
                                .ToList();
                            if (!all.Any()) { await command.FollowupAsync("📭 尚無資料。"); break; }

                            var chunks = new List<string>();
                            var currentChunk = new StringBuilder($"##  點數總覽 ({GetDepartmentDisplayName(botConfig)})\n```ansi\n");
                            ...
                            foreach (var u in all)
                            {
                                ...
                                string pointRaw = $"T:{u.EP + u.OEP} E:{u.EP} O:{u.OEP}".PadLeft(18);
                    ...
                    case "my-info":
                        {
                            ...
                            string targetName = ExtractRobloxName(targetUser);
                            var departmentPoint = await GetDepartmentPointSnapshotAsync(db, botConfig, gid, targetName);

                            var logs = await db.PointLogs.Where(l => l.GuildId == gid && l.RobloxUsername.ToLower() == targetName.ToLower() && !l.IsDeleted)
                                .OrderByDescending(l => l.Timestamp).Take(10).ToListAsync();

                            var sb = new StringBuilder();
                            sb.AppendLine($"### 👤 **{targetName}** 的個人資訊");
                            sb.AppendLine($"> **目前部門：** `{GetDepartmentDisplayName(botConfig)}`");
                            sb.AppendLine($"> **部門點數：** `{FormatPointBreakdown(departmentPoint)}`");
                            sb.AppendLine();
                            ...
                            else
                            {
                                ...
                                {
                                    string pointType = NormalizePointType(l.PointType);
                                    ...
                                    sb.AppendLine($"{idStr} [30m{timeStr}[0m | [{pointType}] ➔ {ptsStr} | 登記: {adminStr} | 原因: {l.Reason}");
                                }
                                sb.AppendLine("```");
                            }

                            sb.AppendLine();
                            sb.AppendLine($"> *數據抓取時間：{Program.GetTaipeiTime():yyyy-MM-dd HH:mm:ss}*");
                    ...
                    case "sync-status":
                        {
                            string requestId = command.Data.Options.First(x => x.Name == "request_id").Value?.ToString() ?? string.Empty;
                            var syncRequest = await db.PointSyncRequests.FirstOrDefaultAsync(x => x.GuildId == gid && x.RequestId == requestId);
                            if (syncRequest == null)
                            {
                                await command.FollowupAsync($"❌ 找不到同步請求 `{requestId}`。");
                                break;
                            }
                            ...
                            {
                                "📡 **同步請求狀態**",
                                $"> 請求編號：`{syncRequest.RequestId}`",
                                $"> 狀態：**{syncRequest.Status}**",
                                $"> 目標玩家：**{syncRequest.RobloxUsername}** ({syncRequest.RobloxUserId})",
                                $"> 部門：**{syncRequest.DepartmentName}**",
                                $"> 點數：**{syncRequest.Amount:+0;-0;0} {syncRequest.PointType}**",
                                $"> 建立時間：{syncRequest.CreatedAt:yyyy-MM-dd HH:mm:ss}"
                            };

                            if (!string.IsNullOrWhiteSpace(syncRequest.ErrorMessage)) lines.Add($"> 失敗原因：{syncRequest.ErrorMessage}");
                            if (syncRequest.PickedAt.HasValue) lines.Add($"> Roblox 拉取時間：{syncRequest.PickedAt.Value:yyyy-MM-dd HH:mm:ss}");
                            if (syncRequest.CompletedAt.HasValue) lines.Add($"> 完成時間：{syncRequest.CompletedAt.Value:yyyy-MM-dd HH:mm:ss}");
                    ...
                    case "addpoint":
                        {
                            ...
                            string pointType = NormalizePointType(command.Data.Options.FirstOrDefault(x => x.Name == "point_type")?.Value?.ToString());
                            string reason = (string)command.Data.Options.First(x => x.Name == "reason").Value;
                            string name = ExtractRobloxName(user);
                            ...
                            var resolved = await TryResolveRobloxUserIdAsync(name);
                            if (!resolved.success) { await command.FollowupAsync($"❌ 無法解析 Roblox UserId：{resolved.error}"); break; }

                            var syncRequest = await CreatePointSyncRequestAsync(db, gid, GetDepartmentKey(botConfig), ResolveDepartmentName(GetDepartmentKey(botConfig)), resolved.userId, name, pointType, pts, reason, command.User.Username, command.User.Id, "addpoint");

                            await command.FollowupAsync($@"✅ **已建立點數同步請求**
> 目標玩家：{name}
> 點數：+{pts} {pointType}
> 原因：{reason}
> 請求編號：`{syncRequest.RequestId}`
> 狀態：等待 Roblox 套用");
                            ...
                            _ = BroadcastToAdminChannelsAsync(gid, $@"負責人 **{command.User.Username}** 已建立點數同步請求
> 部門：**{ResolveDepartmentName(GetDepartmentKey(botConfig))}**
> 被登記人：**{name}**
> 點數：**+{pts} {pointType}**
> 原因：{reason}
> 請求編號：`{syncRequest.RequestId}`
> 狀態：等待 Roblox 套用", pts >= 100);
                            break;
                        }
                    case "removepoint":
                        {
                            ...
                            var syncRequest = await CreatePointSyncRequestAsync(db, gid, GetDepartmentKey(botConfig), ResolveDepartmentName(GetDepartmentKey(botConfig)), resolved.userId, name, pointType, -pts, reason, command.User.Username, command.User.Id, "removepoint");
...
> 狀態：等待 Roblox 套用");
                            ...
                            _ = BroadcastToAdminChannelsAsync(gid, $@"負責人 **{command.User.Username}** 已建立扣點同步請求
> 部門：**{ResolveDepartmentName(GetDepartmentKey(botConfig))}**
> 被登記人：**{name}**
> 點數：**-{pts} {pointType}**
> 原因：{reason}
                    ...
                    case "history":
                        {
                            ...
                            string hName = ExtractRobloxName(hUser);
                    ...
                    case "del-record":
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(targetLog.PendingRevertRequestId)) { await command.FollowupAsync($"⚠️ 該紀錄已有待處理撤銷請求：`{targetLog.PendingRevertRequestId}`。"); break; }

                            var resolved = await TryResolveRobloxUserIdAsync(targetLog.RobloxUsername);
                            if (!resolved.success) { await command.FollowupAsync($"❌ 無法解析 Roblox UserId：{resolved.error}"); break; }

                            string revertPointType = NormalizePointType(targetLog.PointType);
                            decimal revertAmount = -targetLog.PointsAdded;
                            string logDepartmentKey = string.IsNullOrWhiteSpace(targetLog.DepartmentKey) ? GetDepartmentKey(botConfig) : targetLog.DepartmentKey;
                            string syncReason = $"撤銷紀錄 #{logId} - 原因 {delReason}";

                            var syncRequest = await CreatePointSyncRequestAsync(db, gid, logDepartmentKey, ResolveDepartmentName(logDepartmentKey), resolved.userId, targetLog.RobloxUsername, revertPointType, revertAmount, syncReason, command.User.Username, command.User.Id, "del-record-revert", logId);
                            targetLog.PendingRevertRequestId = syncRequest.RequestId;
                            targetLog.RevertRequestedAt = Program.GetTaipeiTime();
                            await db.SaveChangesAsync();

                            string reverseChange = $"{(revertAmount > 0 ? "+" : string.Empty)}{revertAmount} {revertPointType}";
                            await command.FollowupAsync($@"✅ **已建立撤銷同步請求**
> 原紀錄 ID：{logId}
> 反向處理：{reverseChange}
> 原因：{delReason}
> 請求編號：`{syncRequest.RequestId}`
> 狀態：等待 Roblox 套用");
                            ...
                            _ = BroadcastToAdminChannelsAsync(gid, $@"負責人 **{command.User.Username}** 已建立紀錄撤銷同步請求
> 原紀錄：`#{logId}`
> 人員：{targetLog.RobloxUsername}
> 反向處理：{reverseChange}
> 請求編號：`{syncRequest.RequestId}`
> 原因：{delReason}
> 狀態：等待 Roblox 套用", true);
                            break;
                        }
                    ...
                    case "daily-records":
                        {
                            ...
                            foreach (var l in dLogs)
                            {
                                ...
                                string ptsStr = $"{(l.PointsAdded > 0 ? "+" : string.Empty)}{l.PointsAdded}";

                                dSb.AppendLine($"{idStr} {timeStr} | [{pointType}] {l.RobloxUsername} ➔ {ptsStr}點 | 登記: {l.AdminName} | 原因: {l.Reason}");
                            }
                            dSb.AppendLine("```");
                    ...
                    case "points":
                        {
                            ...
                            await command.FollowupAsync($" **{targetName}** 在 **{GetDepartmentDisplayName(botConfig)}** 目前擁有 **EP {departmentPoint.EP} / OEP {departmentPoint.OEP} / 合計 {GetDepartmentTotal(departmentPoint)}**。\n>  *數據抓取時間：{fetchTime}*");
                            break;
                        }
                    ...
                    case "admin-setup":
                        {
                            ...
                            await db.SaveChangesAsync();
                            await command.FollowupAsync($"✅ **已將此頻道設為國防部推播頻道！**\n📡 授權監控的單位編號：`{codes}`\n👮 授權查詢身分組：{roleMsg}\n> *(擁有上述身分組的長官，現在可以在此頻道使用 `/admin-view` 指令查詢各單位排行榜了！)*");
                            break;
                        }
                    case "add-menu-item":
                        {
                    ...
                    case "admin-view":
                        {
                            ...
                            string targetDepartmentKey = GetDepartmentKey(targetConfig);
                            var targetData = (await db.DepartmentPoints.Where(u => u.GuildId == targetConfig.GuildId && u.DepartmentKey == targetDepartmentKey).ToListAsync())
                                .OrderByDescending(u => u.EP + u.OEP)
                                .ThenBy(u => u.RobloxUsername)
                                .ToList();
                            ...
                            targetSb.AppendLine($"## 國防部查閱：單位 [{targetCode}] / {GetDepartmentDisplayName(targetConfig)} 點數總覽");
                            targetSb.AppendLine("```ansi");
                            int tRank = 1;
                            foreach (var u in targetData)
                            {
                                string tRankStr = $"[34m[{tRank}][0m".PadRight(14);
                                string tPointStr = $"[32mT:{u.EP + u.OEP} E:{u.EP} O:{u.OEP}[0m".PadLeft(28);
                    ...
                    case "apply-points":
                        {
                            ...
                            var modal = new ModalBuilder()
                                .WithTitle("點數核發申請單")
                                .WithCustomId($"modal_user_apply_add_{pointType}")
                    ...
                    case "apply-redeem":
                        {
                            ...
                            var modal = new ModalBuilder()
                                .WithTitle(" 點數兌換申請單")
                                .WithCustomId($"modal_user_apply_rem_{pointType}")
        ...
        private async Task UpdateEventDashboardAsync(IDiscordInteraction interaction, string sessId, SocketGuild guild, bool isFirst = false)
        {
            ...
            sb.AppendLine($"> **預定點數：** `{pending.Points} {pending.PointType}`");
            ...
            if (interaction is SocketMessageComponent component)
            {
                ...
                if (id.StartsWith("clear_step2_"))
                {
                    ...
                    if (parts.Length >= 4 && ulong.TryParse(parts[3], out ulong firstAdminId))
                    {
                        ...
                        foreach (var departmentPoint in db.DepartmentPoints.Where(p => p.GuildId == gid))
                        {
                            departmentPoint.EP = 0;
                            departmentPoint.OEP = 0;
                        }
                ...
                if (id.StartsWith("batchok_"))
                {
                    ...
                    await component.UpdateAsync(m => { m.Content = $"⏳ 正在建立批次同步請求 `#{sessId}`，請稍候..."; m.Components = null; });
                    ...
                    foreach (var uid in pending.FinalUserIds)
                    {
                        ...
                        if (!await VerifyUserInRobloxGroup(rName, botConfig.RobloxGroupId)) { failedNames.Add(displayName); continue; }
                        var resolved = await TryResolveRobloxUserIdAsync(rName);
                        if (!resolved.success) { failedNames.Add($"{displayName}(ID解析失敗)"); continue; }
                        var requestItem = await CreatePointSyncRequestAsync(db, gid, pending.DepartmentKey ?? GetDepartmentKey(botConfig), ResolveDepartmentName(pending.DepartmentKey ?? GetDepartmentKey(botConfig)), resolved.userId, rName, pending.PointType, pending.Points, $"[批次#{sessId}] {pending.Reason}", executor.Username, executor.Id, "batch-addpoint");
                        successNames.Add(displayName);
                        requestIds.Add(requestItem.RequestId);
                    }

                    _pendingDistributions.Remove(sessId);
                    string namesJoined = successNames.Any() ? string.Join("、", successNames) : "無";
                    string requestRangeText = requestIds.Any() ? $"`{requestIds.First()}` ~ `{requestIds.Last()}`" : "無成功請求";
                    string resultMsg = $@"✅ **已建立批次同步請求！ (編號：#{sessId})**
>  **受領人員：** {namesJoined}
>  **共建立：** {requestIds.Count} 筆請求
>  **每人獲得：** {pending.Points} {pending.PointType}
>  **原因備註：** {pending.Reason}
>  **執行長官：** {executor.Username}
>  **請求範圍：** {requestRangeText}
>  **狀態：** 等待 Roblox 套用";
                    if (failedNames.Any()) resultMsg += $@"
⚠️ **建立失敗：** {string.Join(", ", failedNames)}";
                    await component.Channel.SendMessageAsync(resultMsg);
                    _ = BroadcastToAdminChannelsAsync(gid, $@"負責人 **{executor.Username}** 已建立批次同步請求 `#{sessId}`
> 活動原因：**{pending.Reason}**
> **名單：{namesJoined}**
> 共建立：**{requestIds.Count}** 筆
> 每人獲得：**{pending.Points} {pending.PointType}**
> 狀態：等待 Roblox 套用", pending.Points >= 100);
                    return;
                }
                ...
                if (id.StartsWith("evok_"))
                {
                    ...
                    await component.UpdateAsync(m => { m.Content = "⏳ 正在建立語音活動同步請求並推播，請稍候..."; m.Components = null; });
                    int successCount = 0;
                    List<string> failedNames = new();
                    List<string> participantNicknames = new();
                    List<string> requestIds = new();
                    ...
                    foreach (var uid in pending.FinalUserIds)
                    {
                        ...
                        var resolved = await TryResolveRobloxUserIdAsync(rName);
                        if (!resolved.success) { failedNames.Add($"{rName}(ID解析失敗)"); continue; }
                        var requestItem = await CreatePointSyncRequestAsync(db, gid, pending.DepartmentKey ?? GetDepartmentKey(botConfig), ResolveDepartmentName(pending.DepartmentKey ?? GetDepartmentKey(botConfig)), resolved.userId, rName, pending.PointType, pending.Points, pending.Reason, executor.Username, executor.Id, "voice-event");
                        requestIds.Add(requestItem.RequestId);
                        successCount++;
                    }
...
⚠️ **建立失敗：** {string.Join(", ", failedNames)}";
                    ...
                    string broadcastMsg = $@"負責人 **{executor.Username}** 已建立語音活動批次同步請求
>  活動：**{pending.Reason}**
>  總時長：**{durationStr}**
>  參與人數：**{successCount}** 人
>  **人員名單：{namesJoined}**
...
>  狀態：等待 Roblox 套用";
            ...
            else if (interaction is SocketModal modal)
            {
                ...
                app.PointType = NormalizePointType(modal.Data.CustomId.Split('_').LastOrDefault());
                app.DepartmentKey = GetDepartmentKey(config);

                if (modal.Data.CustomId.StartsWith("modal_user_apply_add"))
                {
                    app.Type = "發放";
                ...
                else if (modal.Data.CustomId.StartsWith("modal_user_apply_rem"))
                {
                    app.Type = "兌換";
                ...
                var embed = new EmbedBuilder()
                    ...
                    .AddField("金額", $"{app.Amount} {app.PointType}", true)
...
> 申請編號：#{appId}";
            ...
            if (name.Contains("]")) name = name.Substring(name.LastIndexOf(']') + 1).Trim();
            if (!await VerifyUserInRobloxGroup(name, botConfig.RobloxGroupId))
            {
                await component.RespondAsync($"❌ 玩家 `{name}` 不在綁定的 Roblox 群組內。", ephemeral: true);
                return;
            }
            ...
            if (!resolved.success)
            {
                await component.RespondAsync($"❌ 無法解析 Roblox UserId：{resolved.error}", ephemeral: true);
                return;
            }

            string pointType = NormalizePointType(app.PointType);
            decimal change = app.Type == "發放" ? app.Amount : -app.Amount;
            string source = app.Type == "發放" ? "apply-add-approved" : "apply-redeem-approved";
            string reason = $"[申請核准] {app.Reason}";
            var syncRequest = await CreatePointSyncRequestAsync(db, app.GuildId, app.DepartmentKey ?? GetDepartmentKey(botConfig), ResolveDepartmentName(app.DepartmentKey ?? GetDepartmentKey(botConfig)), resolved.userId, name, pointType, change, reason, $"系統審核({component.User.Username})", component.User.Id, source);

            await component.UpdateAsync(m => {
                m.Content = $"✅ **申請已核准並建立同步請求**
> 審核人：{component.User.Mention}
> 申請人：{applicant.Mention} ({name})
> 變動：{change:+0;-0;0} {pointType}
> 原因：{app.Reason}
> 請求編號：`{syncRequest.RequestId}`
> 狀態：等待 Roblox 套用";
                m.Embeds = null; m.Components = null;
            ...
            _ = BroadcastToAdminChannelsAsync(app.GuildId, $@"已核准 **{name}** 的申請並建立同步請求
> 動作：{app.Type} {app.Amount} {pointType}
> 原因：{app.Reason}
> 請求編號：`{syncRequest.RequestId}`
> 狀態：等待 Roblox 套用", Math.Abs((int)change) >= 100);
            _pendingApplications.Remove(appId);
        }
        ...
        private async Task<(int added, int removed)> SyncGroupMembersAsync(BotDbContext db, ulong guildId, string groupId)
        {
            ...
            {
                var removedNames = finalDeleteList.Select(u => u.RobloxUsername.ToLower()).ToHashSet();
                var departmentRows = await db.DepartmentPoints.Where(p => p.GuildId == guildId).ToListAsync();
                var departmentRowsToDelete = departmentRows.Where(p => removedNames.Contains(p.RobloxUsername.ToLower())).ToList();
                if (departmentRowsToDelete.Any()) db.DepartmentPoints.RemoveRange(departmentRowsToDelete);
        ...
        {
            await GoogleSheetSyncHelper.RunAsync(_configuration, guildId);
        }
    }
    ...
    public class BotDbContext : DbContext
    {
        ...
        public DbSet<DepartmentPoint> DepartmentPoints { get; set; }
        ...
        public DbSet<PointSyncRequest> PointSyncRequests { get; set; }
        public DbSet<PointSyncAudit> PointSyncAudits { get; set; }
    ...
    public class DepartmentPoint { [Key] public int Id { get; set; } public ulong GuildId { get; set; } public string RobloxUsername { get; set; } public string DepartmentKey { get; set; } public int EP { get; set; } public int OEP { get; set; } }

    public class PointLog { [Key] public int Id { get; set; } public ulong GuildId { get; set; } public string RobloxUsername { get; set; } public string AdminName { get; set; } public decimal PointsAdded { get; set; } public string? PointType { get; set; } public string? DepartmentKey { get; set; } public string Reason { get; set; } public DateTime Timestamp { get; set; } public bool IsDeleted { get; set; } public string? SyncRequestId { get; set; } public string? MirrorSource { get; set; } public string? PendingRevertRequestId { get; set; } public DateTime? RevertRequestedAt { get; set; } public DateTime? RevertedAt { get; set; } }
    public class PointSyncRequest
    {
        ...
        public string PointType { get; set; } = "EP";
        public decimal Amount { get; set; }
        public string Reason { get; set; } = "";
        public string EditorName { get; set; } = "";
        public ulong? EditorDiscordUserId { get; set; }
        ...
        public int? RelatedPointLogId { get; set; }
    }
    public class PointSyncAudit
    {
        [Key]
    ...
    public class PointSyncAckPayload
    {
        public string RequestId { get; set; } = "";
        public bool Success { get; set; }
        public string? Message { get; set; }
        ...
        public decimal? TotalOEP { get; set; }
    }

    public static class PointSyncMirrorHelper
    {
        public static async Task ApplyMirrorUpdateFromAckAsync(BotDbContext db, IConfiguration config, PointSyncRequest request, PointSyncAckPayload ack)
        {
            var userPoint = await db.UserPoints.FirstOrDefaultAsync(u => u.GuildId == request.GuildId && u.RobloxUsername.ToLower() == request.RobloxUsername.ToLower());
            if (userPoint == null)
            {
                userPoint = new UserPoint { GuildId = request.GuildId, RobloxUsername = request.RobloxUsername, Points = 0 };
                db.UserPoints.Add(userPoint);
            }

            var departmentPoint = await db.DepartmentPoints.FirstOrDefaultAsync(p => p.GuildId == request.GuildId && p.DepartmentKey == request.DepartmentKey && p.RobloxUsername.ToLower() == request.RobloxUsername.ToLower());
            if (departmentPoint == null)
            {
                departmentPoint = new DepartmentPoint { GuildId = request.GuildId, RobloxUsername = request.RobloxUsername, DepartmentKey = request.DepartmentKey, EP = 0, OEP = 0 };
                db.DepartmentPoints.Add(departmentPoint);
            }

            if (ack.TotalEP.HasValue) departmentPoint.EP = (int)Math.Round(ack.TotalEP.Value, MidpointRounding.AwayFromZero);
            if (ack.TotalOEP.HasValue) departmentPoint.OEP = (int)Math.Round(ack.TotalOEP.Value, MidpointRounding.AwayFromZero);
            userPoint.Points = departmentPoint.EP + departmentPoint.OEP;

            db.PointLogs.Add(new PointLog
            {
                GuildId = request.GuildId,
                RobloxUsername = request.RobloxUsername,
                AdminName = request.EditorName,
                ...
                SyncRequestId = request.RequestId,
                MirrorSource = request.Source,
                IsDeleted = false
            });

            if (request.Source == "del-record-revert" && request.RelatedPointLogId.HasValue)
            {
                var originalLog = await db.PointLogs.FirstOrDefaultAsync(x => x.Id == request.RelatedPointLogId.Value && x.GuildId == request.GuildId);
                if (originalLog != null)
                {
                    originalLog.IsDeleted = true;
                    originalLog.RevertedAt = Program.GetTaipeiTime();
                    originalLog.PendingRevertRequestId = request.RequestId;
                    if (!originalLog.Reason.Contains("撤銷完成"))
                    {
                        originalLog.Reason += $" (撤銷完成：{request.RequestId})";
                    }
                }
            }
        }
    ...
    public static class GoogleSheetSyncHelper
    {
        private static int GetDepartmentTotal(DepartmentPoint point) => (point?.EP ?? 0) + (point?.OEP ?? 0);
        private static string GetDepartmentKey(BotConfig? config) => string.IsNullOrWhiteSpace(config?.RobloxGroupId) ? "UNBOUND" : config!.RobloxGroupId.Trim();

        public static async Task RunAsync(IConfiguration configuration, ulong guildId)
        {
            try
            {
                using var db = new BotDbContext(configuration);
                ...
                string departmentKey = GetDepartmentKey(config);
                var users = await db.UserPoints.Where(u => u.GuildId == guildId).ToListAsync();
                var departmentPoints = await db.DepartmentPoints.Where(p => p.GuildId == guildId && p.DepartmentKey == departmentKey).ToListAsync();
                var departmentDict = departmentPoints.ToDictionary(p => p.RobloxUsername.ToLower(), p => p);

                var rankedUsers = users
                    .Select(u => new
                    {
                        User = u,
                        DepartmentPoint = departmentDict.TryGetValue(u.RobloxUsername.ToLower(), out var dp) ? dp : new DepartmentPoint { GuildId = guildId, RobloxUsername = u.RobloxUsername, DepartmentKey = departmentKey, EP = 0, OEP = 0 }
                    })
                    .OrderByDescending(x => GetDepartmentTotal(x.DepartmentPoint))
                    .ThenBy(x => x.User.RobloxUsername)
                    .ToList();

                var valueRange = new ValueRange();
                var oblist = new List<IList<object>>()
                {
                    new List<object>() { "職位", "人員名稱", "EP", "OEP", "部門總分", "最後變動時間" }
                };

                string updateTime = Program.GetTaipeiTime().ToString("yyyy-MM-dd HH:mm:ss");
                foreach (var row in rankedUsers)
                {
                    oblist.Add(new List<object>() { row.User.RobloxRank ?? "尚未同步", row.User.RobloxUsername, row.DepartmentPoint.EP, row.DepartmentPoint.OEP, GetDepartmentTotal(row.DepartmentPoint), updateTime });
                }
                ...
                {
                    await service.Spreadsheets.Values.Clear(new ClearValuesRequest(), individualSheetId, "A:F").ExecuteAsync();
                ...
                Console.WriteLine($"✅ 伺服器 {guildId} 的 Google Sheet 鏡像同步成功。");
            }
            catch (Exception ex)
            {
